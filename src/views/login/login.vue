...
export default {
  ...
  data () {
    return {
      ...
      active_form:
      {
        serial_number:'bcf43a10-e9f0-4ebb-b358-8cfc8c7dbb16',
        company:'',
        active_code:''
      }
    }
  ...
  methods:
  {
    ...
    async doLogin()
    {
      ...
      let sixnum = this.randomRange(6)
      let sessionid = this.sessionid
      let username = encodeURIComponent(this.form.username)
      let userp = this.encrypt(this.form.password,sixnum)
      let code = encodeURIComponent(this.form.verifyCode)
      let v_dx = sixnum + ";userp"
      const result = await loginapi.doLogin(username, userp,sessionid,code,v_dx)
      console.log(result)
    },
    async refreshCode()
    {
      ...
      {
        this.sessionid = this.createRandomString()
      }
    }
  },
  created()
